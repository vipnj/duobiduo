...
package com.duobiduo.midi.editor
{
	...
	import com.duobiduo.midi.InvalidMidiError;
	import com.duobiduo.midi.event.TrackEvent;
	import com.duobiduo.utils.History;
	...
	public class TrackEditor extends EventDispatcher{
		...
		*/
		public function TrackEditor(messageList:MessageList=null):void{
			...
			_globalHistory = History.getGlobalHistory();
		...
		public function set activeMsgList(msgList:MessageList):void{
			...
			dispatchEvent( new TrackEvent( TrackEvent.APPLY_TRACK ) );
		}
		...
		*/
		...
		/**
		* Adjust a block of selected items.  
		* @param	list @see com.duobiduo.midi.model.MessageList
		* @param	v Vertical adjust pitch.
		* @param	h Horizontal adjust timeline.
		*/
		public function adjust( list:MessageList , v:int=0 , h:int=0 ):void{
			if( v!=0 || h!=0 ){
		...
		* @param	item @see com.duobiduo.midi.model.MessageItem 
		*/
		...
		*/
		public function undo():void{
			if ( _globalHistory.pointer != 0 )
			{
				//switch state of cached session **
				for each( var item:MessageItem in _globalHistory.data ){
					item.mark = !item.mark;
				}
				//update the views to redraw
				update( _globalHistory.data );
				//move iterator back
				_globalHistory.undo();
			}
		}
		...
		*/
		public function redo():void{
			if( _globalHistory.pointer!= _globalHistory.tail ){
				//move forth
				_globalHistory.redo();
				//switch state of cached session **
				for each( var item:MessageItem in _globalHistory.data ){
		...
		* @see #execute()
		* @see com.duobiduo.midi.model.MessageItem#mark
		*/
		private function update( arr:Array ):void{
			pending = arr;
			dispatchEvent( new TrackEvent( TrackEvent.UPDATE_TRACK ) );
