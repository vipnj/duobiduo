package com.duobiduo.audio 
{
	...
	{
		[Embed(source="SolaHaXe.swf", mimeType="application/octet-stream")] static private const SWF: Class;
		private const STOP:uint = 0;
		private const PAUSE:uint = 1;
		private const PLAY:uint = 2;
		...
		{   
			...
			else
			{
				...
				if ( !_shouldStop )
				{
					...
					if ( _speed < 1.1)
					{
						_audioBuffer = $sola.process($Sa+$addmax);
					}
					else
					{
						_audioBuffer = $sola.process($addmax);
					}
					trace($sola.getKm());
		...
		protected function initAlchemy():void
		{
			...
			{
				Sola = ldr.contentLoaderInfo.applicationDomain.getDefinition( "Main" ) as Class;
				$sola = new Sola();
				$sola.config(2048, 1);
				$addmax = $sola.getAddmax();
				_audioBuffer = $sola.getPointer();
				trace("load main",_audioBuffer.position);
			}
