package com.duobiduo.audio 
{
	...
	import com.duobiduo.audio.filters.TDSI.SolafsProcess;
	...
	import com.duobiduo.audio.filters.TDSI.SampleFilterCollection;
	...
	public class AudioChannelX 
	{
		...
		private var _status:uint = STOP;			//当前播放状态
		private var _lastSpeed:Number = 1;			//上次设置的速度
		private var _swithing:Boolean = false;		//是否处于速度切换状态
		
		//solafs算法所需变量

		private var $sola:SolafsProcess;		//solafs算法处理器
		private var $Sa:uint=0;					//读取步长
		private var $addmax:uint = 0;			//有效数据长度

		//常量
		public static const SAMPLES_LENGTH:uint = 4096;							//DSP处理固定长度，由SDK所限的最小长度2048,最大长度8192，取合理值4096
		...
		private var _sampleFilters:Array = [];
		private var _bufferFilters:Array = [];
		private var _sampleFiltersCollection:SampleFilterCollection;
		...
		public function get position():Number 
		{
			...
			if (_speed==1.0 && !checkFilters() && _status == PLAY )
			{
		 ...
		 * 处理每个采样的过滤器列表
		 */
		public function get sampleFilters():Array
		{
			normalSoundSwitchFilter();
			return _sampleFilters;
		}
		/**
		 * 当播放速度为一的时候，如果更新了滤镜列表，就需要该方法激活滤镜，
		 * @return 当滤镜列表为空的时返回false
		 */
		private function normalSoundSwitchFilter():void
		{
			_swithing = true;
		 ...
		 * 处理缓冲数据的过滤器列表
		 */
		public function get bufferFilters():Array
		{
			normalSoundSwitchFilter();
			return _bufferFilters;
		}
		...
		{
			if (_sampleFilters.length == 0 && _bufferFilters.length == 0)
			{
				return false;
			}
			if (_sampleFilters.length > 0 || _bufferFilters.length > 0)
		...
		public function AudioChannelX(sound:Object) 
		{
			...
			_sound = sound;
			_dynSnd = new Sound();
			_audioBuffer.position = $sola.entryPosition;
		}
		...
		{
			if ( _speed == 1 && !checkFilters() && _status==PLAY)
			{
				_samplePos = _soundChannel.position * SAMPLE_RATE;
			}
		...
		private function onSampleData(event:SampleDataEvent):void
		{
			...
			{ 
				...
				{
					event.data.writeDouble(_audioBuffer.readDouble());
				}
		...
		private function initSampleFilters():void
		{
			if (null == _sampleFiltersCollection)
			{
				_sampleFiltersCollection = new SampleFilterCollection(this.sampleFilters, SAMPLES_LENGTH);
				_bufferFilters.push(_sampleFiltersCollection);
			}
		}
		
		private function filterBuffer(buffer:ByteArray):void
		{
			for each(var filter:* in _bufferFilters)
			{
				filter.entryPosition = _audioBuffer.position; 
				_audioBuffer.position = filter.process();
			}			
		}
		...
		protected function initAlchemy():void
		{
			...
			_audioBuffer = AutoMemPool.buffer;
			//Alchemy calculates the lookuptables.
			$sola = new SolafsProcess(SAMPLES_LENGTH);
